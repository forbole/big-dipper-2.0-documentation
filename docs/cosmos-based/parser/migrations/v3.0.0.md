...
2. If your chain implements custom messages that are different to standard cosmos messages, you will need to update the custom account parser **(optional)**.

To update custom account parser follow the steps below: 
1. Fork [Juno](https://github.com/forbole/juno) repository.
1. Clone forked juno repository to your local machine.
```shell
$ git clone https://github.com/forbole/juno
$ cd juno/
```
2. Open juno repository in code editor of your choice and add your custom account parser inside [`juno/database/migrate/utils/utils.go`](https://github.com/forbole/juno/blob/cosmos/v0.44.x/database/migrate/utils/utils.go#L3)
```go
...
```
3. Commit and push the changes to github and copy the commit hash.
4. Change directory back to bdjuno
```shell
$ cd ~/bdjuno
``` 
5. Open `go.mod` file and replce the juno version with the commit hash you just copied.
5. Run `go mod tidy` to reload the packages
6. Run `make install` to install the latest changes 
