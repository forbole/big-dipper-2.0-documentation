...
You will need to mofidy 
because `partition_size` and `partition_batch` values will need to be modified before starting the migration. Read [Database Config Reference](./../config/config.md#database) for more information.
...
### Stop BDJuno 
Now you should stop BDJuno by running `CTRL + C`, or if you are running it as a system service run:
```shell
...
### Start BDJuno 
:::warning  **Do __NOT__ start BDJuno yet if you're using PostgreSQL database and you are going to perform table migration. Proceed to the next step instead.**  
::: 
...
```
2. Update partition values inside config.yaml file  
```yaml
database:
