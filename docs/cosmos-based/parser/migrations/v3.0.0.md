...
This will create additional sections inside `.yaml` file for table partition and hasura actions config.  
Don't panic if you see the message pops up: `partition batch size is set to 0, skipping migration` as this is expected.  
You will need to modify values of newly added `partition_size` and `partition_batch` sections before starting the migration. Make sure you set the both values to `0` if you are not using PostgreSQL database and want to skip migration part. Read [Database Config Reference](./../config/config.md#database) for more information.
...
### Stop BDJuno 
Now you can stop BDJuno by running `CTRL + C`, or if you are running it as a system service run:
```shell
...
2. Add custom address parser inside `bdjuno/database/migrate/utils.go` file, 
or leave it empty if no custom address parser is needed. (optional)
```go
...
### Start the migration
:::warning  **Make sure BDJuno is stopped before starting the migration.**   
::: 
